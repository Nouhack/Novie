...
export default withRouter(function Details(props) {
  ...
  useEffect(() => {
    axios
      .get(
        `https://www.omdbapi.com/?apikey=fa3e154a&i=${props.match.params.imdbid}`
      )
      .then((res) => {
  ...
  return (
    <>
      {Loading ? (
        DetailData.Title ? (
          <Row className="h-100 " style={{ backgroundColor: "transparent" }}>
            <>
              <Col xs={12} md={6} lg={4} className=" align-self-center mt-3">
                <img
                  src={
                    DetailData.Poster == "N/A"
                      ? ` https://fakeimg.pl/300x450/131211/11998e/?text=NO IMAGE`
                      : DetailData.Poster
                  }
                  style={{ border: "2px solid #11998e" }}
                ></img>
              </Col>
              <Col
                style={{
                  ...
                  borderRadius: 20,
                  padding: 10,
                  border: "double 5px #11998e",
                }}
                xs={12}
                md={6}
                lg={8}
                className="align-self-center text-left mt-4"
              >
                <ul className="list">
                  ...
                  {DetailData.Type === "series" ? (
                    <h5>Total Seasons : {DetailData.totalSeasons}</h5>
                  ) : null}
                </ul>
              </Col>
            </>{" "}
